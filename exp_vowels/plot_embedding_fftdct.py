...
This is a modified version of plot_embedding.py

2018-08-03
- You can make plotter based on 'center' frame or 'all' frame options
  from make_spkr_dict_myFeatures.py
'''
...
from make_spkr_dict_myFeatures import hp
...
def make_tsv(save_dir, spkr_keys, spkr_id=None):
    ...
    with open(os.path.join(save_dir, 'meta.tsv'), 'w') as f:
        ...
        for s in spkr_keys:
            ...
            i = 0
            for v in phones:
                if sdict[s][v].shape[0] == len(cdict[s][v]):
                    # For each data point
                    for r in range(sdict[s][v].shape[0]):
                        ctx = cdict[s][v][r]
                        f.write('\t'.join(data + [v] + [ctx]) + '\n')
                elif sdict[s][v].shape[0] > len(cdict[s][v]):
                    # if context=='all',
                    for r in range(sdict[s][v].shape[0]):
                        ctx = cdict[s][v][i]
                        f.write('\t'.join(data + [v] + [ctx]) + '\n')
                    ...
                    raise(f'n_frames are smaller than context dictionary')
...
if __name__ == '__main__':
    ...
    # Combine all data
    _x = np.array([], dtype=np.float32).reshape(0, hp.ndct)
    sdict_cent = {s: np.array([], dtype=np.float32).reshape(0, hp.ndct)
                  for s in spkr_keys}
    meta_spkr_vowel = []
    for s in spkr_keys:
        for v in phones:
