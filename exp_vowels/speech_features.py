...
class SpeechFeatures:
    ...
    def get_fft(self, nfft=None, linfilt=False, nfilt=None):
        ...
        self.frames = self._get_frames_windowed()
        # Compute spectrogram
    ...
    ###### Librosa #####
    def librosa_mfcc(self, sig=None, ndct=None):
        ...
        if not ndct:
            ...
            ndct = 12
        # mfcc
        mfcc = librosa.feature.mfcc(y=sig, sr=srate, n_mfcc=ndct).T
        mfcc -= (np.mean(mfcc, axis=0) + 1e-8)
        return mfcc

    ###### python_speech_features #####
    def python_speech_feature_mfcc(self, sig=None, ndct=None):
        ...
        if not ndct:
            ...
            ndct = 12
        # mfcc
        mfcc = pysf_mfcc(sig, srate, self.win_size,
                         self.win_step, ndct, appendEnergy=False)
        return mfcc
